...
def parse_args(arguments):
    ...
    client_method.add_argument('--arp', help='Print ARP Table', action='store_true'
    client_method.add_argument('--all', help='Run all the checks', action='store_true')
...
def ifStatus(ip):
	if args.ifstatuscheck or args.all:
...
def cpuStatusCheck(ip)
	if args.cpustatuscheck or args.all:
...
def envStatus(ip):
	if args.envstatuscheck or args.all:
...
def ifErrors(ip):
	if args.iferrorcheck or args.all:
...
def listAllPhysicalInterfaces(ip):
	if args.iflistphys or args.all:        
...
def listAllVirtualInterfaces(ip):
	if args.iflistvirt or args.all:
...
def listPhysicalAndVirtualInterfacesUp(ip):
	if args.iflistallup or args.all:
...
def printAllInterfaceStatuses(ip):
	if args.ifstatuses or args.all:
...
def printOnlyUpUpInterfaces(ip):
	if args.ifstatusup or args.all:
...
def printListOfStackPorts(ip):
	if args.liststackports or args.all:
...
def printCSListOfUpStackPorts(ip):
	if args.listupstackcs or args.all:
...
def printListOfTrunkPorts(ip):
	if args.listtrunkports or args.all:
...
def printCSListofUpTrunkPorts(ip):
	if args.listuptrunkcs or args.all:
...
def arptable(ip):
	if args.arp or args.all:
		subprocess.call("snmpbulkwalk -v2c -c SNMPCOMMUNITYHERE {} .1.3.6.1.2.1.3.1.1.2".format(ip).split())

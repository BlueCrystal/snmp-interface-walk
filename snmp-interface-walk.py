...
#Runs basic SNMP Test to see if the SNMP Community has been injected in to the device.
        ...
        testsnmp = subprocess.call("snmpget -v2c -c ip14@att {} 1.3.6.1.2.1.1.1.0".format(ip).split())
        if testsnmp == 0:
...
#Runs SNMP Check for Interface Status OIDs. Uses the check_ifstatus nagios script.
        ...
        ifstatus = subprocess.call("/usr/lib/nagios/plugins/check_ifstatus -v2 -H {} -C ip14@att".format(ip).split())

#Runs SNMP Check for CPU Status. Uses the SNMP Load nagios script.
        print("\033[1m" + "CPU Status Sample Test" + "\033[0m")
        cpustatus = subprocess.call("/usr/lib/nagios/plugins/check_snmp_load.pl -H {} -T cisco -C ip14@att -w 101,101,90 -c 102,102,95\n\n".format(ip).split())
...
#Runs SNMP Check for Environment Status. Uses the SNMP Environment nagios script.
        ...
        envstatus = subprocess.call("/usr/lib/nagios/plugins/check_snmp_env.pl -H {} -T cisco -C ip14@att\n\n".format(ip).split())
...
#Runs Memory Test. Uses the Cisco Memory SNMP Script for nagios. Uses a Warning Level of 20% and Critical Level of 5%
        ...
        memorytest = subprocess.call("/usr/lib/nagios/plugins/check_snmp_cisco_memfree -h {} -s ip14@att -w 20 -c 5".format(ip), shell=True)

#Runs Interface Error Test. Uses snmpget.
        print("\033[1m" + "Interface Error Test" + "\033[0m")
        crcinerrors = subprocess.call("snmpget -v2c -c ip14@att {} .1.3.6.1.4.1.9.2.2.1.1.12.3".format(ip), shell=True)
        ignoredinerrors = subprocess.call("snmpget -v2c -c ip14@att {} .1.3.6.1.4.1.9.2.2.1.1.15.3".format(ip), shell=True)
        runtinerrors = subprocess.call("snmpget -v2c -c ip14@att {} .1.3.6.1.4.1.9.2.2.1.1.10.3".format(ip), shell=True)
        frameinerrors = subprocess.call("snmpget -v2c -c ip14@att {} .1.3.6.1.4.1.9.2.2.1.1.13.3".format(ip), shell=True)
        giantinerrors = subprocess.call("snmpget -v2c -c ip14@att {} .1.3.6.1.4.1.9.2.2.1.1.11.3".format(ip), shell=True)
        overruninerrors = subprocess.call("snmpget -v2c -c ip14@att {} .1.3.6.1.4.1.9.2.2.1.1.14.3".format(ip), shell=True)
        collisioninerrors = subprocess.call("snmpget -v2c -c ip14@att {} .1.3.6.1.4.1.9.2.2.1.1.25.3".format(ip), shell=True)

#Prints the current Status of All Interfaces
        print("\033[1m" + "Printing Status of All Interfaces" + "\033[0m")
        intlistallstatus = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.2.1.2.2.1.8 > intlistallstatus && sed -i 's/INTEGER: 1/Up/g' intlistallstatus && sed -i 's/INTEGER: 2/Down/g' intlistallstatus && sed -i 's/INTEGER: 3/Testing/g' intlistallstatus && sed -i 's/INTEGER: 4/Unknown/g' intlistallstatus && sed -i 's/INTEGER: 5/NotPresent/g' intlistallstatus && sed -i 's/INTEGER: 7/LowerLayerDown/g' intlistallstatus".format(ip), shell=True)
...
#Prints the Full List of Interface Descriptions
        ...
        intlistdesc = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.2.1.31.1.1.1.18 > intlistdesc".format(ip), shell=True)
...
#Prints the Full List of Up/Up Physical and Virtual Interfaces
        ...
        intlisten = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.2.1.2.2.1.8 | grep 'INTEGER: 1' > intlist".format(ip), shell=True)
...
#Prints the Full List of Interfaces with their Type (Serial, Fa, Gi, etc)
        ...
        intlistfull = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.2.1.2.2.1.2".format(ip).split())
...
#Prints the Full List of Physical Interfaces Regardless of whether it's down or not
        ...
        intlistphys = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.2.1.2.2.1.2 | grep 'FastEthernet\|GigabitEthernet\|Serial\|Ethernet\|Cellular\|Stack\|TenGigabitEthernet\|ATM'".format(ip), shell=True)
...
#Prints the Full List of Virtual Interfaces regardless of whether it's down or not
        ...
        intlistvirt = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.2.1.2.2.1.2 | grep 'Multilink\|Port-channel\|Vlan\|Embedded-Service-Engine\|Loopback'".format(ip), shell=True)

#Prints a Full List of Stack Ports.
        print("\033[1m" + "Printing All Stack Ports" + "\033[0m")
        stackportlist = subprocess.call("snmpget -v2c -c ip14@att {} iso.3.6.1.4.1.9.5.1.3.1.1.1".format(ip), shell=True)
...
#Prints the full list of Up/Up Interfaces
        ...
        intlistupstatus = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.2.1.2.2.1.8 > intlistupstatus".format(ip), shell=True)
...
#Prints a Full List of Stack Ports
        ...
        stacklist = subprocess.call("snmpbulkwalk -v2c -c ip14@att {} 1.3.6.1.4.1.9.9.500.1.2.2.1.1 > intliststackstatus".format(ip), shell=True)
...
def ciscoserial(ip):
    ...
    subprocess.call("snmpget -v2c -c ip14@att {} iso.3.6.1.2.1.1.1.0 | grep 'Cisco IOS'".format(ip), shell=True)
    subprocess.call("snmpget -v2c -c ip14@att {} iso.3.6.1.2.1.47.1.1.1.1.11.1".format(ip).split())
    subprocess.call("snmpget -v2c -c ip14@att {} iso.3.6.1.2.1.47.1.1.1.1.11.1001".format(ip).split())

def hpserial(ip):
    subprocess.call("snmpget -v2c -c ip14@att {} 1.3.6.1.4.1.11.2.36.1.1.2.2.0".format(ip).split())
